...
temperature_in = FuzzyAssociativeMemory::FuzzyLinguisticVariable.new("room temperature") # degrees fahrenheit
...
fan_speed = FuzzyAssociativeMemory::FuzzyLinguisticVariable.new("fan speed") # cubic feet per minute (CFM)
...
system = FuzzyAssociativeMemory::FuzzyRuleset.new("HVAC control", :larsen)

rule_1 = FuzzyAssociativeMemory::FuzzyRule.new('If room is cold, the fan motor stops',           [cold],       nil, stop)
rule_2 = FuzzyAssociativeMemory::FuzzyRule.new('If room is cool, the fan motor is slow',         [cool],       nil, slow)
rule_3 = FuzzyAssociativeMemory::FuzzyRule.new('If room is just right, the fan motor is medium', [just_right], nil, medium)
rule_4 = FuzzyAssociativeMemory::FuzzyRule.new('If room is warm, the fan motor speeds up',       [warm],       nil, fast)
rule_5 = FuzzyAssociativeMemory::FuzzyRule.new('If room is hot, the fan motor runs full-blast',  [hot],        nil, blast)
